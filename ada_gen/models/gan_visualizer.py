...
class GANVisualizer():
    ...
    def exportVisualization(self,
        ...
        while remaining > 0:
            ...
            print('noiseData.shape',noiseData.shape)
...
#             print('img',img)
    ...
    def exportDB(self, path, nItems):
        ...
        while remaining > 0:
            ...
            noise_list.append(noiseData.cpu().numpy())
            ...
            remaining -= currBatch
        noise_array = np.concatenate(noise_list, axis=0)
        noise_tensor = torch.from_numpy(noise_array)

# 保存为 .pt 文件
        torch.save(noise_tensor, os.path.join(path, 'noise.pt'))
#         np.save(os.path.join(path, 'noise.npy'), noise_array)
