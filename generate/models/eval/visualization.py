...
#     if checkpointData is None:
#         raise FileNotFoundError(
#             "Not checkpoint found for model " + name + " at directory " + directory)
    ...
    print('interpolationPath=====+',interpolationPath)
    if interpolationPath is not None:
        print('interpolationPath',interpolationPath)
        interpolationVectors = torch.load(interpolationPath)
        pathOut = os.path.splitext(interpolationPath)[0] + "_interpolations10cha150"
        print('interpolationVectors===',interpolationVectors)
    elif kwargs['random_interpolate']:
        interpolationVectors, _ = visualizer.model.buildNoiseData(3)
        pathOut = os.path.splitext(pathModel)[0] + "_interpolations10cha150"
        print('interpolationVectors',interpolationVectors)
    if interpolationVectors is not None:
        ...
        for img in range(nImgs):
            ...
            path = os.path.join(pathOut, str(img))
